...
class EmbeddingAlgorithm:
    """
    The knowledge base embedding algorithm.
    """

    def __init__(self, dao, root_topic_names, max_depth, phrase_cache):
        """
       :param dao: the DAO used to match phrases to resources and traverse the topic hierarchy.
       :param root_topic_names: set of the names of the root topics in the topic hierarchy.
       :param max_depth: the maximum depth we are permitted to traverse upwards from the leaf nodes.
       :param phrase_cache: cache of phrases that actually exist in the knowledge base so we can avoid DB lookups.
        """
        self.dao = dao
    ...
    def identify_topic_probabilities(self, text):
        ...
        :returns: a dict containing topic name to root topic probability.
        """
    ...
    def identify_leaf_nodes(self, text):
        ...
        resource matches in the knowledge base.
        It is assumed the text has had stopwords removed and can be tokenised by splitting on
        whitespace.
    ...
    def identify_topics(self, resource):
        """
        Given a resource, looks up the resource in the knowledge base to determine its immediate topics.
        
        :param resource: the resource to lookup
        :returns: the list of topics for the resource.
        """
    ...
    def get_topic_probabilities(self, depth):
        """
        Gets the topic probabilities from the topic tree generated by the last call to identify_topic_probabilities().
    ...
    def get_all_topic_probabilities(self):
        ...
        :returns: a dict of topic to probability, all topics in the tree are returned.
        """
    ...
    def remove_parents_where_no_path_to_root(self, parent_topics, steps_left):
        """        
        Apply the topic filtering algorithm.  Ensure any paths that do not lead to a root topic in the given
        number of steps left are removed so vote can be reallocated to valid paths.
        
        :param parent_topics: the candidate parent topics vote can be allocated to.
        :param steps_left: the maximum number of steps left for us to reach a root topic.
        :returns: only the parent topics that are on a valid path to a root topic in the steps left.
        """
        filtered_parents = []
    ...
    def get_or_add_topic_to_cache(self, topic_name, cache, depth):
        """        
        Enables a cache to be maintained of traversed topics for the last identify_topic_probabilities() run.
        This then enables get_topic_probabilities() and get_all_topic_probabilities() to return topic probabilities.
        This method also applies the root topic propogation algorithm carrying a root topic forward if it appears
        at a lower level of the tree.
        
        :param topic_name: the topic name to add or get from the cache.
        :param cache: the traversed topic cache.
        :param depth: the depth of the tree the topic appeared at.
        :returns: the topic added or obtained to the cache.
        """
